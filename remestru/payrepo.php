...
$p=0;
	...
	$conde="AND (`account`.`aville` = '$custo')";
		...
		if($p)
$doj=mysqli_query($conn, "SELECT `payment`.* FROM `payment` INNER JOIN `account` ON `payment`.`Partis` = `account`.`number` WHERE `payment`.`Date` BETWEEN '$dati' AND '$datis' AND `account`.`province`='$province' $conde ORDER BY `payment`.`Date` ASC, `payment`.`number` ASC");
		else
$doj=mysqli_query($conn, "SELECT *FROM (SELECT `payment`.* FROM `payment` INNER JOIN `account` ON `payment`.`Partis` = `account`.`number` WHERE `account`.`province`='$province' ORDER BY `payment`.`Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC, `number` ASC");
		 ...
		 <div class="input-group-addon"><span class="lnr lnr-calendar-full"></span>
			...
			<table class="table table-striped table-hover table-sm" style="font-size:8px;">     
                    ...
                    <tr role="row"><th> Date </th><th> Code </th><th><div align='center'> Ville/Territoire </th>
                        <th> Nom & Prenom </th><th> Cantact </th><th style="text-align:center;"> Mode de Paiement </th>
						<th><div align='center'> Référence </th><th><div align='center'>Cash</th>
						<th><div align='center'>Banque</th><th><div align='center'>Cheque</th></tr>
                    </thead><tbody>
		<?php
					$i=1;				$tam=$tpa=$tba=0;		
...
print("<tr><td $stl><div align='center'>$date&nbsp;</td><td $stl><div align='center'>$ucode&nbsp;</td><td $stl> $ville </td>
	<td $stl>$nom $postnom $prenom</td><td class='text-right' $stl> $contact </td><td $stl> &nbsp;$pline </td>
	<td class='text-left' $stl> &nbsp;$refo </td><td $stl><div align='right' style='padding-right:20px;'> $toto </td>
	<td $stl><div align='right' style='padding-right:20px;'> $amoo </td><td $stl><div align='right' style='padding-right:20px;'> $resto </td></tr>");
		...
		$tam=number_format($tam, 2);					$tpa=number_format($tpa, 2);				$tba=number_format($tba, 2);
				...
				<th class="text-right" style='padding-right:20px;'><?php echo $tba ?></th></tr>
