<?php
if(basename($_SERVER['PHP_SELF']) == 'regrepo.php') 
  $cm=" class='current'";
include'header.php';
...
if(isset($_POST['search']))
	...
	if($p)
$doj=mysqli_query($conn, "SELECT *FROM `account` WHERE `Status`!='1' AND `province`='$province' AND `Rdate` BETWEEN '$dati' AND '$datis' $conde ORDER BY `Rdate` ASC");
	else
		 ...
		 <div class="input-group-addon"><span class="lnr lnr-calendar-full"></span>
			...
			<table class="table table-striped table-hover table-sm style="font-size:8px;">     
					...
					<th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Code&nbsp;&nbsp;</th><th>&nbsp;&nbsp;&nbsp;Date&nbsp;&nbsp;</th>
						...
						<th> Ville/Territoire </th><th> Etat-Civil </th></tr>
                    </thead><tbody>
		<?php
					$n=1;			
...
$ville=$ro['aville'];
$civil=$ro['civil'];
$date=date("d-m-Y", strtotime($ro['Rdate']));
$ucode=$ro['Code'];
$file=$ro['photo'];
	if($file)
...
print("<tr><td $stl><div align='center'>$ucode&nbsp;</td><td $stl><div align='center'> $date </td>
	...
	<td $stl> $ville </td><td $stl> $civil </td></tr>");
			$n++;		
	}
