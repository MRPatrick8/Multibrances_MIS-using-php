<?php
if(basename($_SERVER['PHP_SELF']) == 'versrepo.php') 
  $cm=" class='current'";
include'header.php';
...
$p=0;
	...
	$conde="AND (`account`.`aville` = '$custo')";
		...
		if($acco)
	$condi="AND (`payment`.`Bank` = '$acco')";
		
		if($p)
$doj=mysqli_query($conn, "SELECT `payment`.* FROM `payment` INNER JOIN `account` ON `payment`.`Partis` = `account`.`number` WHERE `payment`.`Date` BETWEEN '$dati' AND '$datis' AND `account`.`province`='$province' AND `payment`.`Pline`='Dépôt bancaire' $conde $condi ORDER BY `payment`.`Date` ASC, `payment`.`number` ASC");
		else
$doj=mysqli_query($conn, "SELECT *FROM (SELECT `payment`.* FROM `payment` INNER JOIN `account` ON `payment`.`Partis` = `account`.`number` WHERE `account`.`province`='$province' AND `payment`.`Pline`='Dépôt bancaire' ORDER BY `payment`.`Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC, `number` ASC");
  ...
  <div class="row">
        ...
        <div class="col-lg-10">
		 ...
		 <?php
          ...
          <div class="col-md-2"><select class="form-control" name="acco" style="padding-left:5px; padding-right:5px;">
			...
			while($roi=mysqli_fetch_assoc($doi)){
				...
				if($acco==$nu)
					...
					$c="";
			echo"<option value='$nu' title='$purpo' $c> $fna $acc </option>";
			}
			?>   
                            </select></div>
		 ...
		 <div class="input-group-addon"><span class="lnr lnr-calendar-full"></span>
			...
			<table class="table table-striped table-hover table-sm" style="font-size:8px;">     
                    ...
                    <tr role="row"><th> Date </th><th> Code </th><th><div align='center'> Ville/Territoire </th>
                        <th> Nom & Prenom </th><th> Cantact </th><th><div align='center'> Numéro de bordereau </th>
						<th><div align='center'> Numéro de compte </th><th><div align='center'>Nom de la Banque</th>
						<th><div align='center'>Montant Payé</th></tr>
                    </thead><tbody>
		<?php
					$i=1;				$tam=$tpa=$tba=0;		
...
print("<tr><td $stl><div align='center'>$date&nbsp;</td><td $stl><div align='center'>$ucode&nbsp;</td><td $stl> $ville </td>
	<td $stl>$nom $postnom $prenom</td><td class='text-right' $stl> $contact </td><td $stl> &nbsp;$cheno </td>
	<td class='text-left' $stl><div align='right'> $accosee </td><td $stl><div align='left'> $bansee </td>
	<td $stl><div align='right' style='padding-right:20px;'> $toto </td></tr>");
			$i++;							$tam+=$tot;					
	}
		$tam=number_format($tam, 2);					
				?></tbody><thead>
				<tr><th> </th><th colspan='3'>&nbsp;&nbsp;&nbsp; Montant Total </th><th colspan='4'> </th>
