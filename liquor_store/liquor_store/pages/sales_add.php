...
include('../dist/includes/dbcon.php');
	...
	$cid=$_REQUEST['cid'];
		...
		mysqli_query($con,"INSERT INTO sales(cust_id,user_id,discount,amount_due,mode,total,date_added,modeofpayment,cash_tendered,cash_change,branch_id) 
	VALUES('$cid','$id','$discount','$amount_due','$mode','$total','$date','cash','$tendered','$change','$branch')")or die(mysqli_error($con));
	...
	$query=mysqli_query($con,"select * from temp_trans where branch_id='$branch'")or die(mysqli_error($con));
		...
		$query1=mysqli_query($con,"SELECT or_no FROM payment NATURAL JOIN sales  ORDER BY payment_id DESC LIMIT 0 , 1")or die(mysqli_error($con));
			...
			$row1=mysqli_fetch_array($query1);
				...
				$orn = uniqid();
				mysqli_query($con,"INSERT INTO payment(cust_id,user_id,payment,payment_date,branch_id,payment_for,status,sales_id,or_no) 
	VALUES('$cid','$id','$total','$date','$branch','$date','paid','$sales_id','$orn')")or die(mysqli_error($con));
	...
	$remarks="sold $qty of bottles at $price";  
